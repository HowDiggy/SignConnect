...
def test_create_question_for_scenario(
    ...
    response = authenticated_client.post(
        f"/api/users/me/scenarios/{parent_scenario.id}/questions/", json=question_data
    )
    ...
    db_question = (
        db_session.query(models.ScenarioQuestion)
        .filter(models.ScenarioQuestion.id == uuid.UUID(question_id))
        .one_or_none()
    )
...
def test_cannot_create_question_for_another_users_scenario(
    ...
    scenario_owned_by_b = crud.create_scenario(
        ...
        scenario=schemas.ScenarioCreate(
            name="User B's Private Scenario", description="A test"
        ),
        user_id=user_b.id,
    )
...
def test_update_question(authenticated_client: TestClient, db_session: Session):
    """
    ...
    initial_question = crud.create_scenario_question(
        ...
        question=schemas.ScenarioQuestionCreate(
            question_text="Old Question", user_answer_text="Old Answer"
        ),
    ...
    response = authenticated_client.put(
        f"/api/users/me/questions/{initial_question.id}", json=update_data
    )
...
def test_cannot_update_another_users_question(
    ...
    question_b = crud.create_scenario_question(
        db=db_session,
        question=schemas.ScenarioQuestionCreate(
            question_text="User B's Question", user_answer_text="An answer"
        ),
    ...
    response = authenticated_client.put(
        f"/api/users/me/questions/{question_b.id}", json=update_data
    )
...
def test_delete_question(authenticated_client: TestClient, db_session: Session):
    """
    GIVEN an authenticated user with an existing question,
    ...
    question_to_delete = crud.create_scenario_question(
        db=db_session,
        question=schemas.ScenarioQuestionCreate(
            question_text="Delete Me", user_answer_text="An answer"
        ),
        scenario_id=scenario.id,
    )
    ...
    assert (
        db_session.query(models.ScenarioQuestion)
        .filter(models.ScenarioQuestion.id == question_id)
        .one_or_none()
        is not None
    )
