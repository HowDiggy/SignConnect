...
def read_scenarios_for_user(
    ...
    if not db_user:
        print("--- 3. User not found in local DB. Returning empty list. ---")
        return []
...
def create_scenario(
    ...
    existing_scenario = crud.get_scenario_by_name(
        db, name=scenario.name, user_id=db_user.id
    )
    if existing_scenario:
        raise HTTPException(
            status_code=400, detail="A scenario with this name already exists."
        )
...
def create_scenario_question(
    ...
    if db_scenario.user_id != db_user.id:
        raise HTTPException(
            status_code=403, detail="Not authorized to add questions to this scenario."
        )
    return crud.create_scenario_question(
        db=db, question=question, scenario_id=scenario_id
    )
...
def delete_scenario(
    ...
    *,
    ...
    deleted_scenario = crud.delete_scenario_by_id(
        db=db, scenario_id=scenario_id, user_id=db_user.id
    )
